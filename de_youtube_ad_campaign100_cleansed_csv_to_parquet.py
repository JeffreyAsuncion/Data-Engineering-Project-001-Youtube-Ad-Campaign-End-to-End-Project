...
## @type: DataSource
## @args: [database = "db_youtube_ad_campaign100_raw", table_name = "raw_statistics", transformation_ctx = "datasource0"]
## @return: datasource0
## @inputs: []
############################### Added for predicate pushdown (filter for english language data) ###############################
predicate_pushdown = "region in ('ca','gb','us')"

datasource0 = glueContext.create_dynamic_frame.from_catalog(database = "de_youtube_ad_campaign100_raw", table_name = "raw_statistics", transformation_ctx = "datasource0", push_down_predicate = predicate_pushdown)
###############################################################################################################################
...
## @type: DataSink
## @args: [connection_type = "s3", connection_options = {"path": "s3://de-youtube-ad-campaign100-cleansed-useast1-dev/youtube/raw_statistics/"}, format = "parquet", transformation_ctx = "datasink4"]
## @return: datasink4
...
datasink4 = glueContext.write_dynamic_frame.from_options(frame = df_final_output, connection_type = "s3", connection_options = {"path": "s3://de-youtube-ad-campaign100-cleansed-useast1-dev/youtube/raw_statistics/", "partitionKeys": ["region"]}, format = "parquet", transformation_ctx = "datasink4")
